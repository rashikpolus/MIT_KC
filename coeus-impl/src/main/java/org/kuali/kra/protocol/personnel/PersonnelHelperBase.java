...
public abstract class PersonnelHelperBase implements Serializable {
    ...
    protected void initializePersonUnits() {
        int totalPersons = getProtocol().getProtocolPersons().size();
        int totalPersonUnits = newProtocolPersonUnits.size();
        if(newProtocolPersonUnits.isEmpty()) {
            for(int personIndex = 0;  personIndex < totalPersons; personIndex++) {
                   this.newProtocolPersonUnits.add(personIndex, createNewProtocolUnitInstanceHook());
             }
        }else if(totalPersonUnits < totalPersons) {
            for(int personIndex = totalPersonUnits;  personIndex < totalPersons; personIndex++) {
                this.newProtocolPersonUnits.add(personIndex, createNewProtocolUnitInstanceHook());
            }
        }
     }

...
public class TimeAndMoneyAction extends KcTransactionalDocumentActionBase {
    ...
    private void captureDateChangeTransactions(ActionForm form) throws WorkflowException {
        ...
        captureDateChanges(timeAndMoneyDocument, awardHierarchyNodeItems);
    }

	void captureDateChanges(TimeAndMoneyDocument timeAndMoneyDocument, final List<AwardHierarchyNode> awardHierarchyNodeItems) {
        ...
        updateAwardAmountTransactions(timeAndMoneyDocument);
        Map<String, Award> activeAwards = awardVersionService.getAllActiveAwardsForHierarchy(timeAndMoneyDocument.getAwardNumber()).stream().collect(Collectors.toMap(Award::getAwardNumber, award -> award));
        for(Entry<String, AwardHierarchyNode> awardHierarchyNode : timeAndMoneyDocument.getAwardHierarchyNodes().entrySet()){
            Award award = activeAwards.get(awardHierarchyNode.getValue().getAwardNumber());
            if (award == null) {
            	award = awardVersionService.getPendingAwardVersion(awardHierarchyNode.getValue().getAwardNumber());
            }
            int index = findAwardHierarchyNodeIndex(awardHierarchyNode);
            AwardAmountInfo aai = getAwardAmountInfoService().fetchAwardAmountInfoWithHighestTransactionId(award.getAwardAmountInfos());
            boolean needToSaveAward = false;
            String dateChangedComment = null;
			AwardHierarchyNode currentAwardHierarchyNode = awardHierarchyNodeItems.get(index);
            ...
            if(award.getAwardNumber().equals(timeAndMoneyDocument.getAward().getAwardNumber())) {
                ...
                if (mustSetFandADistributions(award.getAwardDirectFandADistributions(),timeAndMoneyDocument.getAward().getAwardDirectFandADistributions())) {
...
//          }
    ...
    public ActionForward docHandler(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(StringUtils.isEmpty(gotoAwardNumber)) {
        	gotoAwardNumber = rootAwardNumber;
        }

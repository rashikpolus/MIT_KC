...
public class ScheduleXmlStream extends PrintBaseXmlStream {
    ...
    private void setAttendance(CommitteeSchedule committeeSchedule, Schedule schedule) {
        Set<String> membersPresent = new HashSet<String>();
        ...
        for (CommitteeScheduleAttendanceBase attendanceInfoBean : attendenceList) {
            ...
            membersPresent.add(getPersonKey(attendanceInfoBean.getPersonId(), attendanceInfoBean.getNonEmployeeFlag()));
         }
        ...
        for (CommitteeMembershipBase committeeMembership : committeeMemberships) {
        	...
        	String personId = committeeMembership.getPersonId();
        	if(personId == null) {
        		personId = committeeMembership.getRolodexId().toString();
        		nonEmployee = true;
        	}
        	String committeeMemberKey = getPersonKey(personId, nonEmployee);
        	if(getMeetingService().isCommitteeActiveMemberAbsent(committeeMembership, committeeSchedule) &&
        			!membersPresent.contains(committeeMemberKey)) {
                Attendents attendents = schedule.addNewAttendents();
    ...
    protected String getPersonKey(String personId, boolean nonEmployee) {
    	return personId.concat(nonEmployee ? PersonTypeConstants.NONEMPLOYEE.getCode() : PersonTypeConstants.EMPLOYEE.getCode());
    }

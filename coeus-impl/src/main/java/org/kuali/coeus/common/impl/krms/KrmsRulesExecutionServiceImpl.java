...
public class KrmsRulesExecutionServiceImpl implements KrmsRulesExecutionService {
    ...
    public List<Map<String,String>> processUnitKcValidations(String unitNumbersAsString, KrmsRulesContext rulesContext) {
        kcKrmsCacheManager.clearCache();
        Map<String, String> contextQualifiers = new HashMap<String, String>();
        rulesContext.populateContextQualifiers(contextQualifiers);
        List<Map<String,String>> combinedErrorList = new ArrayList<Map<String,String>>();
        List<String> unitNumbersSortedList = new ArrayList<String>();
        List<String> unitNumbers = new ArrayList<String>();
        Facts.Builder factsBuilder = Facts.Builder.create();
        rulesContext.addFacts(factsBuilder);
        String[] unitArray = StringUtils.split(unitNumbersAsString, ",");
        if(unitArray!=null){
        ...
        if(rulesContext instanceof ProposalDevelopmentDocument){
        	ProposalDevelopmentDocument pdDocument = (ProposalDevelopmentDocument) rulesContext;
        	if(!unitNumbers.isEmpty()){
        		unitNumbersSortedList = getUnitNumbersInOrder(pdDocument.getDevelopmentProposal().getUnitNumber());
        		unitNumbersSortedList.removeAll(unitNumbers);
        		unitNumbersSortedList.addAll(unitNumbers);
        	}
        }
        ...
        if (engine == null) {
            LOG.error("Could not resolve KRMS Rules Engine - Unit Validations will not be evaluated!");
        	  ...
        	  for (String unit : unitNumbersSortedList) {
        		  SelectionCriteria selectionCriteria = SelectionCriteria.createCriteria(null, contextQualifiers,Collections.singletonMap(KcKrmsConstants.UNIT_NUMBER, unit));
        		  EngineResults results = engine.execute(selectionCriteria, factsBuilder.build(), null);
        		  if (results != null) {

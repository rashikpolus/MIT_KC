declare
	li_count number;
begin
	select count(osp_distribution_code) into li_count from DISTRIBUTION	where osp_distribution_code = -5;
	if li_count = 0 then
		insert into DISTRIBUTION(OSP_DISTRIBUTION_CODE,VER_NBR,DESCRIPTION,UPDATE_TIMESTAMP,UPDATE_USER,OBJ_ID,ACTIVE_FLAG)
		values('-5',1,'dummy',sysdate,'admin',sys_guid(),'Y');
	end  if;
...
commit
/
update AWARD_TEMPLATE_REPORT_TERMS set OSP_DISTRIBUTION_CODE = -5 where OSP_DISTRIBUTION_CODE is null
/
declare
cursor c_templ is
...
/	 
update AWARD_TEMPLATE_REPORT_TERMS set OSP_DISTRIBUTION_CODE = null where OSP_DISTRIBUTION_CODE = -5
/
delete from DISTRIBUTION where OSP_DISTRIBUTION_CODE = -5
/
commit
/
declare

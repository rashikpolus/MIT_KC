...
--
delete from KRIM_PERM_ATTR_DATA_T where PERM_ID in 
		(select perm_id from KRIM_PERM_T where nm='Modify Proposal Development S2s While Enroute')
/
delete from KRIM_ROLE_PERM_T where PERM_ID in 
		(select perm_id from KRIM_PERM_T where nm='Modify Proposal Development S2s While Enroute')
/
delete from KRIM_PERM_T where nm='Modify Proposal Development S2s While Enroute' 									
/
...
INSERT INTO KRIM_ROLE_PERM_T (ROLE_PERM_ID,ROLE_ID,PERM_ID,ACTV_IND,OBJ_ID,VER_NBR) 
    VALUES ('KC' || KRIM_ROLE_PERM_ID_S.NEXTVAL,(SELECT ROLE_ID FROM KRIM_ROLE_T WHERE NMSPC_CD = 'KC-PD' AND ROLE_NM = 'Modify All Dev Proposals'),'KC' || KRIM_PERM_ID_S.CURRVAL,'Y',SYS_GUID(),1);
